...
class ResNet(nn.Module):
    ...
    def __init__(self, nInputChannels, block, layers, os, pretrained=True):
        self.inplanes = 64
...
def ResNet101(nInputChannels, os, pretrained=True):
...
class DeepLabv3(nn.Module):
    def __init__(self, nInputChannels, n_classes, os, pretrained=False, _print=True):
        if _print:
        ...
        self.last_conv_ = nn.Conv2d(256, n_classes, kernel_size=1, stride=1)
        
        # self.selayer = SELayer(in_channels=256, out_channels=256, n_classes=n_classes)
...
if __name__ == "__main__":
    model = DeepLabv3(nInputChannels=3, n_classes=6, os=16, pretrained=True, _print=True)
    model.eval()
